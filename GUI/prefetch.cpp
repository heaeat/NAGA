...
bool get_prefetch_info(list<punknownp> *unknown_list)
{
	...
	//
	if (is_serial_type(unknown_list)) {			// serial 의 형태ㅣㄴ경우
		get_volume_name();
		get_volume_serial();
		parse_volume_serial(unknown_list);
	}
	else {
		get_device_volume();					//HARDDISK VOLuME의 형태인경우
		parse_device_ver(unknown_list);
	}
...
bool is_serial_type(list<punknownp> *unknown_list) {
	for (auto unknown : *unknown_list) {
		wstring id = unknown->id();
		if (id.find(L"DEVICE") != std::string::npos) {
			return false;
		}
	}
	return true;
...
void get_device_volume() {
	...
	for (;;)
	{
		...
		wstring normal_path = volume_path_to_normal(VolumeName);
		to_upper_string(DeviceName);
		wstring device_name = DeviceName;
		device_name.erase(0,1);
		volume_serial_list.insert(pair<wstring, wstring>(device_name, normal_path));
		log_info "%ws, %ws", volume_path_to_normal(VolumeName).c_str(), device_name.c_str() log_end;
		//
		//  Move on to the next volume.
...
void parse_device_ver(list<punknownp> *unknown_list) {
	...
	for (map<wstring, wstring>::iterator serial_iter = volume_serial_list.begin(); serial_iter != volume_serial_list.end(); serial_iter++) {
		for (iter = unknown_list->begin(); iter != unknown_list->end(); iter++)
		{
			...
			//
			//	unknown_list는 이런 형태로 저장되어 있다.
			//	ex)\DEVICE\HARDDISKVOLUME1\WINDOWS\SYSTEM32\WDI\LOGFILES\WDICONTEXTLOG.ETL.003
			stringstream str_id;
			str_id << WcsToMbsUTF8Ex((*iter)->id());
			...
			stringstream str_device;
			str_device << WcsToMbsUTF8Ex(serial_iter->first.c_str());

			int location = (str_id.str()).find(str_device.str());
			if (location != string::npos) {
				int len = str_device.str().length();

				stringstream str_name;
				str_name << WcsToMbsUTF8Ex(serial_iter->second.c_str());

				string temp = str_id.str();
				temp.replace(0, location + len + 1, str_name.str().c_str());
